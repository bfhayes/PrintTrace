...
public class OutlineTool: ObservableObject {
    ...
    public struct ProcessingParams: Sendable {
    ...
    /// Process image to extract contour (async)
    ...
    /// Process image to DXF file (async)
    ...
    private func processImageToContourSync(
        ...
        // C function pointers can't capture context, so we use nil for now
        // Progress updates would need a different approach for Swift integration
        let progressCallback: OutlineToolProgressCallback? = nil
        let errorCallback: OutlineToolErrorCallback? = nil
...
// MARK: - URL Integration Helpers

@available(macOS 10.15, iOS 13.0, *)
extension OutlineTool {
    
    /// Convenient method for file URL processing
